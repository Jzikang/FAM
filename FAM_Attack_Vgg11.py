...
def main():
    ...
    for (name, module) in vgg_11.named_modules():
        ...
        if isinstance(module, nn.MaxPool2d):
            vgg11_layer_list.append(name)
        if isinstance(module, nn.Conv2d):
            vgg11_layer_list.append(name)
        if isinstance(module, nn.Linear):
            vgg11_layer_list.append(name) 
    print(vgg11_layer_list)

    vgg11_layer_list = ["1.features.26"]

    for layer_name in vgg11_layer_list:
        handlers = []
        for (name, module) in vgg_11.named_modules():
            if name == layer_name:
                print(name)
        ...
        for i, (x, y) in (enumerate(val_loader)):
            if i != args.num // args.batchsize:
                ...
                with torch.no_grad():
                    target_y = getTarClas(x, y, vgg_11, args.byRank)
                    furthest_feature = getFurthestFeature(x, vgg_11, target_y, library, args.method, args.kernel_for_furthe, args.GAA, args.c)
                x_adv = attack_fp(x, furthest_feature, vgg_11, args.kernel_type, args.GAA, args.c, args.method).detach_()
                # save adv img
                # save_path = os.path.join(root, str(i))
                # save_img(save_path, x_adv)
                list_18, list_target_y, num, utr, tsuc, ttr = test(x_adv, y, target_y, vgg_11, vgg_19, inc_v3, densenet_201, resnet_152, list_18, list_y, num, utr, tsuc, ttr, device)
                print(tsuc)
            else:
                break
        D_adv = float(args.num)
        print('Error for vgg11: %10.4f' % float(100 * (float(num[0]) / D_adv)))
        print('tSuc for vgg11: %10.4f' % float(100 * (float(tsuc[0]) / D_adv)))
