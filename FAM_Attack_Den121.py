...
def main():
    ...
    inc_res = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('inception_resnet_v2', pretrained=True).eval().to(device))
    ...
    for (name, module) in densenet_121.named_modules():
        ...
        if name == '1.features.norm5':
            layer_list.append(name)
    print(layer_list)
    layer_list = ['1.features.denseblock4.denselayer14']
    for layer_name in layer_list:
        ...
        for i, (x, y) in (enumerate(val_loader)):
            if i != args.num // args.batchsize:
                ...
                list_121, list_y, num, utr, tsuc, ttr = test(x_adv, y, target_y, densenet_121, vgg_19, inc_v3, densenet_201, resnet_152, res_v2_50, mob_v2, resnext_101, list_121, list_y, num, utr, tsuc, ttr, device)
                print(tsuc)
            else:
                break
        ...
        print('Error for dense201: %10.4f' % float(100 * (float(num[3]) / D_adv)))
        print('uTR for dense201: %10.4f' % float(100 * (float(utr[2]) / float(num[0]))))
        print('tSuc for dense201: %10.4f' % float(100 * (float(tsuc[3]) / D_adv)))
        print('tTR for dense201: %10.4f' % float(100 * (float(ttr[2]) / float(tsuc[0]))))
        print('Error for res152: %10.4f' % float(100 * (float(num[4]) / D_adv)))
        print('uTR for res152: %10.4f' % float(100 * (float(utr[3]) / float(num[0]))))
        print('tSuc for res152: %10.4f' % float(100 * (float(tsuc[4]) / D_adv)))
        if tsuc[0]<1e-20:
            print('tTR for res152: %10.4f' % float(100 * (0.0)))
        else:
            print('tTR for res152: %10.4f' % float(100 * (float(ttr[3]) / float(tsuc[0]))))
