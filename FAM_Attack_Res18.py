...
parser.add_argument('--iteration', default=50, type=int, metavar='N', help='number of attack iteration')
parser.add_argument('-b', '--batchsize', default=20, type=int, metavar='N', help='mini-batch size (default: 18)')
...
parser.add_argument('--num', '--data_num', default=1000, type=int, metavar='N', help='the num of test images')
parser.add_argument('-j', '--workers', default=8, type=int, metavar='N',help='number of data loading workers (default: 4)')
parser.add_argument('--kernel_type', type=str, default='poly',  help='the type of kernel')
...
parser.add_argument('--kernel_for_furthe', type=str, default='l_poly', help='kernel for search furthest feature')
...
parser.add_argument('--targetcls', type=int, default=100, help='select the target class indix, 2,10,100,500,1000')
...
class PAA_Loss(nn.Module):
    ...
    def forward(self, source_feature, target_feature):
        if self.kernel == 'gussi':
            PAA_g_ = PAA_g()
            self.loss = PAA_g_(source_feature, target_feature)
...
def PAA_furthest(s, t, kernel, c):
    ...
    for i in range(batch_size):
        distance.append(loss(s[i*batch_size:(i+1)*batch_size],t[i*batch_size:(i+1)*batch_size]))
    #distance = loss(s, t)
    for i in range(batch_size):
        #index.append(torch.argmax(distance[i * 20: (i + 1) * 20], dim=0) + i * 20)
        index.append(torch.argmax(distance[i], dim=0) + i * batch_size)
        #dis, inde = torch.topk(distance[i], 10)
        #index.append(inde[9]+ i * batch_size)
        #index.append(torch.tensor(0) + i * batch_size)
    #print(index)
    if len(t.shape) == 2:
        t = t[:, None, None, :]
    return t[index, :, :, :]
...
def attack_fp(x, t_f, t_y, model, kernel, GAA, c, method):
    batch_size = x.shape[0]
    alpha = args.eps / args.iteration
    momentum = torch.zeros([batch_size, 3, args.size, args.size]).to(device)
    if GAA == 1:
        loss = GAA_Loss(GAA)
    else:
        loss = PAA_Loss(kernel, c)
    for i in range(args.iteration):
        ori_out = model(x)
        ###
        real = ori_out.gather(1,t_y.unsqueeze(1)).squeeze(1)
        ce_loss = -1 * real.sum()
        ###
        s_f = mid_outputs
        if method==1:
            # PAA
            loss_ = loss(s_f, t_f).sum()# + ce_loss/10000000
            #print(loss(s_f, t_f).sum(), ce_loss/10000000)
        elif method==2:
            # GAA
...
def attack_mi(x, t_y, model):
    alpha = args.eps / args.iteration
    momentum = torch.zeros([args.batch_size, 3, args.size, args.size]).to(device)
    for i in range(args.iteration):
        pred_logit = model(x)
        ce_loss = F.cross_entropy(pred_logit.to(device), t_y.to(device), reduction='sum').to(device)
        ce_loss.backward()
        noise = x.grad.data
        l1_noise = torch.sum(torch.abs(noise), dim=(1, 2, 3))
        l1_noise = l1_noise[:, None, None, None]
...
def attack_Ti(x, t_y, model, images_min, images_max):
    alpha = args.eps / args.iteration
    T_kern = torch.from_numpy(gkern(15, 3)).to(device)
    for i in range(args.iteration):
        pred_logit = model(x)
        ce_loss = F.cross_entropy(pred_logit.to(device), t_y.to(device), reduction='sum').to(device)
        ce_loss.backward()
...
def getTarClas(x, y, model, rank=1):
    ...
    if rank==1:
        ...
        target_y = torch.from_numpy(target_y).to(device)
    return target_y
"""
def getFurthestFeature(x, model, target_y, library, method, kernel, GAA, c):
    ...
    for j in target_y:
        target_library.append(library[j])
        target_out = model(library[j].to(device))  
        target_feature.append(mid_outputs)
    target_library = torch.cat(target_library, axis=0)
    ...
    source_feature_tiled = torch.cat(source_feature_tiled, dim=0).to(device)
    if method == 1: 
        # PAA
...
def main():
    ...
    #library = np.load('./ImageNet_gallery_vgg_0.06.npy')
    library = np.load('./ImageNet_gallery.npy')
    ...
    #trn = transforms.Compose([transforms.Resize(224), transforms.CenterCrop(224), transforms.ToTensor(), ])
    trn = transforms.Compose([transforms.ToTensor(), ])
    image_id_list, label_ori_list, label_tar_list = load_ground_truth('./dataset/images.csv')

    img_size = 299
    input_path = './dataset/images/'
    #input_path = './adv_test_0.01/'
    lr = 2 / 255  # step size
    epsilon = 16  # L_inf norm bound
    num_batches = np.int(np.ceil(len(image_id_list) / args.batchsize))
    ...
    """
    resnet_152 = torch.nn.Sequential(Normalize(args.mean, args.std), models.resnet152(pretrained=True).eval().to(device))
    densenet_121 = torch.nn.Sequential(Normalize(args.mean, args.std), models.densenet121(pretrained=True).eval().to(device))
    densenet_201 = torch.nn.Sequential(Normalize(args.mean, args.std), models.densenet201(pretrained=True).eval().to(device))
    resnet_18 = torch.nn.Sequential(Normalize(args.mean, args.std), models.resnet18(pretrained=True).eval().to(device))
    #resnet_34 = torch.nn.Sequential(Normalize(args.mean, args.std), models.resnet34(pretrained=True).eval().to(device))
    #resnet_50 = torch.nn.Sequential(Normalize(args.mean, args.std), models.resnet50(pretrained=True).eval().to(device))
    vgg_19 = torch.nn.Sequential(Normalize(args.mean, args.std), models.vgg19_bn(pretrained=True).eval().to(device))
    #vgg_13 = torch.nn.Sequential(Normalize(args.mean, args.std), models.vgg13_bn(pretrained=True).eval().to(device))
    vgg_11 = torch.nn.Sequential(Normalize(args.mean, args.std), models.vgg11_bn(pretrained=True).eval().to(device))
    inc_v3 = torch.nn.Sequential(Normalize(args.mean, args.std), models.inception_v3(pretrained=True).eval().to(device))
    inc_res_v2 = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('inception_resnet_v2', pretrained=True).eval().to(device))
    mob_vit = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('twins_svt_small', pretrained=True).eval().to(device))
    adv_inc = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('adv_inception_v3', pretrained=True).eval().to(device))
    ens_inc_res_v2 = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('ens_adv_inception_resnet_v2', pretrained=True).eval().to(device))
    res_v2_50 = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('resnetv2_50', pretrained=True).eval().to(device))
    #xcep = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('xception', pretrained=True).eval().to(device))
    #vit = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('vit_base_patch8_224', pretrained=True).eval().to(device))
    ghost = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('ghostnet_100', pretrained=True).eval().to(device))
    mob_v2 = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('mobilenetv2_140', pretrained=True).eval().to(device))
    #resnext_101 = torch.nn.Sequential(Normalize(args.mean, args.std), timm.create_model('resnext101_64x4d', pretrained=True).eval().to(device))
    global mid_outputs
    mid_outputs = None
    res18_layer_list = []
    """
    ...
    #layer_list = ["1.layer4.1"]
    layer_list = ["1.features.16"]
    """
    for (name, module) in vgg_13.named_modules():
        print(name)
    ...
    print(layer_list)
    """
    #layer_list = ['1.features.denseblock4.denselayer16']
    #layer_list = ['1.Mixed_6c']
    for layer_name in layer_list:
        handlers = []
        for (name, module) in vgg_11.named_modules():
            if name == layer_name:
                print(name)
        ...
        for k in range(0, num_batches):
            print(k)
            batchsize_cur = min(args.batchsize, len(image_id_list) - k * args.batchsize)
            X_ori = torch.zeros(batchsize_cur, 3, img_size, img_size).to(device)
            delta = torch.zeros_like(X_ori, requires_grad=True).to(device)#生成和括号内变量维度维度一致的全是零的内容
            for i in range(batchsize_cur):
                X_ori[i] = trn(Image.open(input_path + image_id_list[k * args.batchsize + i] + '.png'))
                #X_ori[i] = trn(Image.open(input_path + '/' + str(k * args.batchsize + i) + '.png'))
            labels = torch.tensor(label_ori_list[k * args.batchsize:k * args.batchsize + batchsize_cur]).to(device)
            target_labels = torch.tensor(label_tar_list[k * args.batchsize:k * args.batchsize + batchsize_cur]).to(device)
            prev = float('inf')
            x = Variable(X_ori.to(device), requires_grad=True)
            with torch.no_grad():
                target_y = getTarClas(x, labels,vgg_11, args.byRank)
                #print(target_y)
                #target_y = torch.tensor([820]*args.batchsize).to(device)
                furthest_feature = getFurthestFeature(x, vgg_11, target_y, library, args.method, args.kernel_for_furthe, args.GAA, args.c)
                
            x_adv = attack_fp(x, furthest_feature, target_y, vgg_11, args.kernel_type, args.GAA, args.c, args.method).detach_()
            # save adv img
            """
            root = "./adv_AA"
            save_path = os.path.join(root, str(k))
            save_img(save_path, x_adv)
            """
            list_18, list_target_y, num, utr, tsuc, ttr = test(x_adv, labels, target_y, vgg_11, vgg_19, densenet_201, resnet_152, inc_v3, inc_res_v2, ghost, mob_vit, list_18, list_y, num, utr, tsuc, ttr, device)
            print(tsuc)
        D_adv = float(1000)
        """
        for i, (x, y) in (enumerate(val_loader)):
            if i != args.num // args.batchsize:
                ...
                with torch.no_grad():
                    target_y = getTarClas(x, y, resnet_18, args.byRank)
                    furthest_feature = getFurthestFeature(x, resnet_18, target_y, library, args.method, args.kernel_for_furthe, args.GAA, args.c)
                x_adv = attack_fp(x, furthest_feature, resnet_18, args.kernel_type, args.GAA, args.c, args.method).detach_()
                # save adv img
                # save_path = os.path.join(root, str(i))
                # save_img(save_path, x_adv)
                list_18, list_target_y, num, utr, tsuc, ttr = test(x_adv, y, target_y, resnet_18, vgg_19, inc_v3, densenet_201, resnet_152, list_18, list_y, num, utr, tsuc, ttr, device)
                print(tsuc)
            else:
                break
        D_adv = float(args.num)
        """
        print('Error for vgg_11: %10.4f' % float(100 * (float(num[0]) / D_adv)))
        print('tSuc for vgg_11: %10.4f' % float(100 * (float(tsuc[0]) / D_adv)))
        ...
        print('Error for dense201: %10.4f' % float(100 * (float(num[2]) / D_adv)))
        print('uTR for dense201: %10.4f' % float(100 * (float(utr[1]) / float(num[0]))))
        print('tSuc for dense201: %10.4f' % float(100 * (float(tsuc[2]) / D_adv)))
        print('tTR for dense201: %10.4f' % float(100 * (float(ttr[1]) / float(tsuc[0]))))
        print('Error for res152: %10.4f' % float(100 * (float(num[3]) / D_adv)))
        print('uTR for res152: %10.4f' % float(100 * (float(utr[2]) / float(num[0]))))
        print('tSuc for res152: %10.4f' % float(100 * (float(tsuc[3]) / D_adv)))
        print('tTR for res152: %10.4f' % float(100 * (float(ttr[2]) / float(tsuc[0]))))
        print('Error for inc_v3: %10.4f' % float(100 * (float(num[4]) / D_adv)))
        print('uTR for inc_v3: %10.4f' % float(100 * (float(utr[3]) / float(num[0]))))
        print('tSuc for inc_v3: %10.4f' % float(100 * (float(tsuc[4]) / D_adv)))
        print('tTR for inc_v3: %10.4f' % float(100 * (float(ttr[3]) / float(tsuc[0]))))
        print('Error for resv2_50: %10.4f' % float(100 * (float(num[5]) / D_adv)))
        print('uTR for resv2_50: %10.4f' % float(100 * (float(utr[4]) / float(num[0]))))
        print('tSuc for resv2_50: %10.4f' % float(100 * (float(tsuc[5]) / D_adv)))
        print('tTR for resv2_50: %10.4f' % float(100 * (float(ttr[4]) / float(tsuc[0]))))
        print('Error for ghost: %10.4f' % float(100 * (float(num[6]) / D_adv)))
        print('uTR for ghost: %10.4f' % float(100 * (float(utr[5]) / float(num[0]))))
        print('tSuc for ghost: %10.4f' % float(100 * (float(tsuc[6]) / D_adv)))
        print('tTR for ghost: %10.4f' % float(100 * (float(ttr[5]) / float(tsuc[0]))))
        print('Error for mobilenet_v2: %10.4f' % float(100 * (float(num[7]) / D_adv)))
        print('uTR for mobilenet_v2: %10.4f' % float(100 * (float(utr[6]) / float(num[0]))))
        print('tSuc for mobilenet_v2: %10.4f' % float(100 * (float(tsuc[7]) / D_adv)))
        print('tTR for mobilenet_v2: %10.4f' % float(100 * (float(ttr[6]) / float(tsuc[0]))))
        for h in handlers:
            h.remove()
